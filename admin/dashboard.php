...
<body class="bg-slate-50 text-slate-800 min-h-full font-sans antialiased flex flex-col m-0 p-0">
    ...
    <header>
        <nav class="fixed top-0 left-0 w-full bg-gradient-to-r from-blue-700 to-red-600 text-white py-3.5 px-4 shadow-lg z-50 backdrop-blur-md bg-opacity-95">
            <div class="container mx-auto max-w-7xl flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <span class="hidden sm:inline-block text-[10px] font-bold uppercase tracking-wider bg-white/20 px-2.5 py-0.5 rounded-full border border-white/10">Admin Panel</span>
                </div>

                <div class="flex flex-wrap items-center justify-center md:justify-end gap-5 w-full md:w-auto">
                    <div class="flex items-center gap-5 text-sm font-medium">
                        <a href="./dashboard.php" class="relative py-1 font-semibold after:absolute after:bottom-0 after:left-0 after:w-full after:h-0.5 after:bg-white transition-all">Dashboard</a>
                    </div>
    ...
    <main class="container mx-auto max-w-7xl pt-28 pb-12 px-4">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-xs border border-slate-200/60 mb-6 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
                ...
                <form method="GET" class="flex items-center gap-2">
                    <div class="relative flex-1">
                        ...
                        <input type="text" name="cari" placeholder="Cari Nama / Tahun Lulus / Jurusan..."
                            ...
                            class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-red-500 focus:bg-white focus:ring-4 focus:ring-red-500/10 transition-all duration-200">
                    </div>
                    ...
                    <?php if (isset($_GET['cari']) && $_GET['cari'] != ''): ?>
                        <a href="dashboard.php" class="bg-slate-100 hover:bg-slate-200 text-slate-600 p-2.5 rounded-xl transition-colors border border-slate-200 flex items-center justify-center" title="Reset Pencarian">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="flex items-center justify-end">
                <a href="./create.php" class="w-full md:w-auto bg-red-600 hover:bg-red-700 text-white font-bold text-sm px-5 py-2.5 rounded-xl shadow-md shadow-red-600/10 transition-all duration-200 flex items-center justify-center gap-2 active:scale-[0.98]">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        while ($data = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<tr class='hover:bg-slate-50/80 transition-colors duration-150 text-sm text-slate-700 font-medium'>
                                 <td class='px-6 py-4 text-center font-bold text-slate-400'>{$data['id_alumni']}</td>
                                 ...
                                 <td class='px-6 py-4'>
                                     <div class='flex items-center justify-center gap-3'>
                                         <a href='./update.php?id={$data['id_alumni']}' class='text-xs font-bold bg-slate-100 hover:bg-slate-200 text-slate-700 py-1.5 px-3 rounded-lg transition-colors border border-slate-200/40'>
                                             Edit
                                         </a>
                                         <a href='./delete.php?id={$data['id_alumni']}' onclick=\"return confirm('Yakin ingin hapus?')\" class='text-xs font-bold bg-red-50 hover:bg-red-100 text-red-600 py-1.5 px-3 rounded-lg transition-colors border border-red-100/40'>
                                             Hapus
                                         </a>
                                     </div>
                                 </td>
    ...
    <footer class="mt-auto shrink-0 w-full bg-slate-900 text-slate-300 border-t border-slate-800/80">
        ...
        <div class="mt-2 py-4 flex-col text-center text-xs text-slate-400 tracking-wide">
            <p class="text-md font-medium">&copy; 2026 <span class="text-slate-500">Iqbal Brillian Taaqwa</span></p>
        </div>
        </div>
    </footer>
